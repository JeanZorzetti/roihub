...
function stableReason(reason) {
  ...
  return /^(?:global-disabled|project-disabled|decision:(?:duplicate|unsafe)|draft:[a-z,-]+|gsc-unavailable|llm-(?:auth|rate|output)|unsplash-(?:auth|rate|output)|github-(?:auth|missing|rate|output|conflict)|render:[a-z:-]+|verification:[a-z-]+|request-failed|invalid-response|invalid-result|verification-timeout|http-\d{3}|unexpected)$/.test(reason)
    ? reason
    : null;
}
