...
- [X] T041 hub: commit e push fora de 23:30–01:00 e 08:00–08:45 BRT (Princípio IV); o push leva junto os 3 commits de docs parados. Depois, conferir a rota em produção com o quickstart §2. **16/09**: `26740e2` publicado às 15:48 BRT; o build no EasyPanel levou ~24 min com 81% de steal e entrou às 16:12. Quickstart §2 em produção: sem segredo 401, `projeto: "x"` 400, `caminho: "//evil.com"` 400, chamada válida 200 `{ enviado: true }`. `next build` local passou antes, para descartar erro de build
- [X] T042 atma: commit e push de T004 e T014–T018; deploy do backend pelo EasyPanel; esperar terminar antes de T043 (build simultâneo derruba a VPS com 137). **16/09**: `c340daf` publicado às 16:13 BRT, depois de o build do hub terminar; o push disparou o deploy sozinho, e o `/api/system/health` voltou `OK` com `uptime` zerado às 16:14. Na execução, `tests/patientController.test.js` ganhou os casos do FR-003 (site avisa sem observações; painel e 409 não avisam). `adminAuth.test.js` e `rateLimiter.test.js` já falhavam no jest antes da 027: são scripts de `node` puro e passam assim
- [X] T043 roilabs: commit e push de T005, T019–T021 e T031–T038; deploy do painel pelo EasyPanel, **depois** de T042 terminar. **16/09**: `2345d93` publicado às 16:16 BRT; o push rebuildou os 3 serviços do repositório em série (`site` 16:28, `goiania` 16:35, `app` 16:52). Depois: `login`, `site` e `goiania` com 200, cancelamento com token inexistente com 404, os 5 serviços tocados com 1/1
- [X] T044 coopluz: commit e push de T006–T007 e T022–T025; conferir o deploy de Production na Vercel. **16/09**: `3fcd19e`; o site serve o build novo (`Last-Modified` 18:55 UTC, 6 min depois do push) e `/api/lead` responde
- [X] T045 **dono**: antes de preencher os segredos, rodar o quickstart §3 (SC-003) na Coopluz e na Atma; depois, preencher `ROIHUB_CRM_SECRET` na Atma, no ROI Labs e na Coopluz (redeploy) e **conferir se existe** na Vértice (quickstart §1, R3). **18/09**: o dono pôs nos 2 serviços do EasyPanel (atma 17/09 21:43, roilabs_app 18/09 11:46). Na Coopluz **faltava** na Vercel: criada e produção redeployada em 18/09. Na Vértice já existia (33 dias), e o lead de teste chegou ao hub (`crm_leads` id 24), o que fecha o risco da R3. O SC-003 não foi repetido: ele já foi provado na 026 com a Sirius, e a falta do segredo só produz uma linha de log
...
- [ ] T047 rodar o quickstart §2, §4, §5 e §6 e registrar aqui SC-001, SC-002, SC-004, SC-005 e SC-006, o caminho usado para pago/devolvido/renovação recusada e o resultado do bot do WhatsApp da Atma (se ele manda `x-admin-key`, os leads dele não avisam — R5). **§2 e §4 feitos em 18/09**: os 6 envios foram aceitos — Vértice pelo formulário real (Playwright, lead id 24), Coopluz `duplicado: false`, Atma paciente id 57 e parceria id 3, ROI Labs candidatura e lead de consumidor 201. Nenhuma linha de falha de aviso nos logs de hub, atma e roilabs_app, com os logs vivos (121 linhas na Atma no período) — ou seja, o hub aceitou os 6. **SC-001 depende de o dono confirmar as 6 mensagens no Telegram.** SC-004: o reenvio da Coopluz voltou `duplicado: true`; o da Atma bateu no limite de 15 min (429) antes do 409, e nos dois casos nada foi avisado. **§5 travado** na T046. **§6**: `roilabs_app` **não tem `CRON_SECRET`** no EasyPanel, então a rota do resumo semanal responde 503 e o cron nunca roda; e a tabela `assinaturas` está vazia, então cancelamento e renovação recusada não têm o que provar. Pago e devolvido seguem para o 1º evento real. O canal do `sendAlert` ficou provado pelos avisos de candidatura e de lead de consumidor, que passam por ele
