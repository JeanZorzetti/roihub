...
- [X] T024 [US3] Validar pelo [quickstart.md](./quickstart.md), incluindo o teste que mais importa: derrubar a escrita de telemetria de propósito e confirmar as **duas** metades da FR-007 ao mesmo tempo — a busca continua respondendo **e** a janela aparece como lacuna, jamais como "zero falhas". **Validado em 10/08** (ver handoff.md): números da aba batem com o SQL cru; a 2ª metade da FR-007 falhou na 1ª tentativa (500 em vez de lacuna) — corrigido com `try/catch` em `app/ia/page.tsx`, revalidado local (banner de lacuna aparece, busca continua 200), `npm test` (327/327) e `tsc --noEmit` limpos. Falta só o check numérico contra um ciclo noturno completo em produção (nenhum rodou ainda — ver T030).
