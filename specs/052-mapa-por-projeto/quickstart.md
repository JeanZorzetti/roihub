...
```bash
git worktree add ../roihub-antes <commit anterior à 052>
# em dois terminais, com o mesmo .env:
(cd ../roihub-antes && npx next dev -p 3001)
npx next dev -p 3002
...
- `curl -sI localhost:3002/gsc/mapa` → `307` com `location: /gsc/mapa/atma` (FR-002).
- Tirando os números, o diff de texto só pode ter as trocas da tabela D5 (cabeçalho, frase do board e
  seletor). Nenhuma folha troca de estado.
...
curl -sI localhost:3002/gsc/mapa/tapepro                   # 404 (FR-003)
```
...
- As 113 folhas estão na lista `#board-lista`.
...
No mapa do Sirius em produção, nenhuma das ~11 folhas que leem o banco diz "nenhuma corrida gravada".
