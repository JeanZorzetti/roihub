...
- [ ] T018 [US1] Conferir na tela servida por `app/okr/[slug]/aquisicao/page.tsx` em `http://localhost:3000/okr/atma/aquisicao` (quickstart §4): hosts nomeados, Índice de Conformidade com denominador avaliando 23 páginas (SC-002), veredito ≈13,04% (3 de 23), nenhuma URL repetida (SC-003), impressões do bloco ÷ 24.664 ≥ 99% (SC-001)
...
- [ ] T024 [US2] Conferir a ficha da Atma em `lib/ficha-dados.ts` renderizada (quickstart §5): a página de maior movimento com as ~22.000 impressões (SC-005), sem aparecer duas vezes, e o número de **páginas necessárias** da camada de entrega mudando junto com as impressões — se só um dos dois mudou, a média ainda está mentindo
...
| 2 | **`gscSeries(p.url)` (D8) contradiz a linha nova na mesma tela.** A série ao vivo lê um host só e alimenta "recebido 6 dos 28 dias", `impressoesV` e `recebidaGsc`. | Tela: "recebido 6 dos 28 dias (2026-09-11 → 2026-09-16) · hosts somados: atma.roilabs.com.br + usealigner.com" na mesma linha. | Fora do escopo por decisão da spec. **Spec própria (031).** Ela também decide a T024. |
| 3 | **T024 não é observável na Atma hoje.** A camada de Entrega está `não apurada — a árvore parou antes da camada de impressões`. Além disso, `impressoesV` (série de um host) e a média por página (agora dois hosts) ficaram em escopos diferentes até a 031. | `/okr/atma/metodo`. | Aberta, depende da 031. |
