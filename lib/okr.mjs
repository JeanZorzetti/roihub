...
export const PERFIS = {
  ...
  D: {
    ...
    n2: "Receita = Leads × CR(lead→orçamento) × CR(orçamento→tratamento) × Valor do tratamento",
    // ⚠️ A cadeia era `consulta AGENDADA → COMPARECEU`, escrita para clínica que marca consulta.
    // A Atma nunca marcou: ela era representante comercial e o que prometia era ACHAR um doutor
    // perto do paciente — promessa que morreu com a saída do sócio comercial (02/09/2026), depois
    // de um lead acusar propaganda enganosa. O posicionamento novo é PREÇO, e o degrau real que
    // sobrou está medido em `orcamentos`, não numa agenda que não existe.
    marcos: [
      { chave: "visitante", nome: "visitante", coletor: "cliques", familia: "D1", fonte: "Search Console" },
      ...
      // `celulaDeContato()` acima: a MESMA linha que já dá `lead` também dá `status`, e a regra
      // que a Atma declarou ("todo cancelado foi contatado") o torna apurável sem coletor novo.
      { chave: "contatado", nome: "contato feito", coletor: "contatados", familia: "D4", fonte: "coluna `status` do próprio projeto, regra declarada: todo lead fora de `novo` foi contatado" },
    ...
    fatores: [
      ...
      { nome: "CR(lead→orçamento)", tipo: "cadeia", cobertura: ["contatado", "orcamento"] },
      // Cobertura é só `["tratamento"]`, não `["orcamento","tratamento"]`: `avaliarN2()` (lib/
      // ficha.mjs) pega o marco ANTERIOR à cobertura como origem da taxa, e "orcamento" já é o
      // fim do fator de cima — repeti-lo aqui contaria o mesmo marco duas vezes e acusaria
      // sobreposição. O nome do fator continua "CR(orçamento→tratamento)" porque é isso que ele
      // calcula: tratamento sobre o orçamento anterior.
