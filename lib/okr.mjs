...
/**
 * Status de `orcamentos` que contam como ACEITE. **Vazio de propósito.**
 *
 * 🚩 Em 02/09/2026 a tabela só conheceu `enviado` (7 linhas, R$ 37.465, todas iguais) e não há
 * código no repo da Atma que escreva outro valor — o `default` do banco é a única escrita. Chutar
 * o enum (`aprovado`? `fechado`? `aceito`?) devolveria **0 aceitos com cara de apurado**, que é
 * precisamente o defeito que esta camada existe para não cometer: um `0` fabricado é pior que um
 * `não apurado`, porque encerra a pergunta em vez de fazê-la.
 *
 * Preencher quando a Atma DECLARAR a regra de aceite — não quando alguém adivinhar.
 */
export const STATUS_ACEITE = new Set();

/**
 * Os dois degraus de orçamento na janela, da MESMA leitura da tabela — uma query, duas células.
 *
 * 🚩 A regra do subprojeto vale igual aqui: zero na história inteira é `não apurado`, nunca `0`.
 * Tabela que nunca recebeu orçamento não separa "ninguém pede preço" de "o app não grava"; tabela
 * que nunca recebeu aceite não separa "ninguém aceita" de "ninguém atualiza o status depois de
 * enviar". As duas hipóteses dão o mesmo zero e pedem trabalho OPOSTO (oferta × encanamento).
 *
 * @param {{criado:string, status:string}[]|null} rows @param {{inicio:string, fim:string}} janela
 */
export function celulasDeOrcamento(rows, { inicio, fim }) {
  if (!Array.isArray(rows)) {
    const sem = naoApurado("sem fonte de orçamento");
    return { enviados: sem, aceitos: sem };
  }
  const naJanela = rows.filter((r) => String(r.criado).slice(0, 10) >= inicio && String(r.criado).slice(0, 10) <= fim);
  return {
    enviados: rows.length
      ? apurado(naJanela.length)
      : naoApurado("tabela `orcamentos` existe e nunca recebeu orçamento — não separa 'ninguém pede preço' de 'o app não grava'"),
    aceitos: STATUS_ACEITE.size
      ? apurado(naJanela.filter((r) => STATUS_ACEITE.has(String(r.status))).length)
      : naoApurado("tabela `orcamentos` existe e nunca recebeu status de aceite — não separa 'ninguém aceita o preço' de 'ninguém atualiza o status depois de enviar'"),
  };
}
...
export const PERFIS = {
  ...
  D: {
    ...
    marcos: [
      ...
      // A `fonte` sai DUAS vezes por linha na tela (no motivo e no "consultar:") — o porquê mora
      // aqui, não nela: `status` é posição ATUAL, não evento, e quem cancelou depois de atendido
      // já não conta como contatado. Por isso a fonte é o histórico, não a coluna.
      { chave: "contatado", nome: "contato feito", coletor: null, familia: "D4", fonte: "`status_historico` do próprio projeto — tabela JÁ existe (migration rodada, trigger armado), mas só tem a posição de HOJE semeada: 0 transições reais gravadas, e contar `para='contatado'` devolveria os mesmos 14 que a coluna `status` e subestimaria igual" },
      { chave: "orcamento", nome: "orçamento ENVIADO", coletor: "orcamentos", familia: "D3", fonte: "tabela `orcamentos` do próprio projeto" },
      // Aceite é etapa PRÓPRIA, não ruído: mandar preço e não fechar é um vazamento inteiro que
      // some quando as duas etapas viram uma só — e é onde a Atma aposta o posicionamento novo.
      { chave: "aceito", nome: "orçamento ACEITO", coletor: "orcamentosAceitos", familia: "D3", fonte: "tabela `orcamentos`, status de aceite" },
    ...
    fatores: [
      ...
      { nome: "CR(orçamento→tratamento)", tipo: "cadeia", cobertura: ["aceito", "tratamento"] },
