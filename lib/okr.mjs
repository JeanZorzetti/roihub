...
export function ticketDeOrcamentos(rows, { inicio, fim }) {
  ...
  return apurado(liquidos.reduce((a, b) => a + b, 0) / liquidos.length);
}
 ...
 * que quem nunca saiu de `novo` nunca foi tocado. Não precisa de coletor novo nem de instrumentar
 * `status_historico` (que segue com 0 transições reais gravadas): a MESMA linha que já dá `lead`
 * também dá `status`.
 *
