...
export function projetar({ ficha, meta, hoje, janelaDias = 28 }) {
  ...
  // A divisão da META (calendário + ticket) começa aqui, ANTES da guarda 8 — 019/FR-008: ela não
  // toca a âncora. `n1Total = meta.valor / meta.ticket` e a normalização por prazo são contas de
  // CALENDÁRIO; suprimi-las por causa da cadeia era zerar sete campos por causa de um só
  // (`validacao_so_fala_de_problema_cala_o_caminho_feliz`).
  ...
  // 8 — âncora zerada (G5), agora PARCIAL (019/FR-008..FR-011). A FR-011 revoga por escrito a
  // FR-034 da 018 ("`lib/projecao.mjs` NÃO DEVE ganhar regra nova"): era regra de spec, não de
  // constituição.
  //
  // As guardas 1–7 continuam devolvendo `naoApuradaCompleta()` inteira porque cada uma nomeia um
  // fator que falta na PRÓPRIA divisão da meta. A 8 é a única que fala da CADEIA — e o que ela
  // suprime é só o que divide POR volume: `fatorObrigatorio` e `multiploNecessario` dividem por
  // `ancora.valor`, que é 0. `n1Total`/`n1Janela` não dividem por nada disso e ficam.
  //
  // `ancora` sai como o OBJETO real (valor 0), não `null` como nas guardas 1–7: a FR-010 exige que
  // os campos suprimidos nomeiem o degrau zerado, e sem o objeto a tela não tem o nome dele.
  if (ancora.valor === 0) {
