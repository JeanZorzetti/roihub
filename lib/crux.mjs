 ...
 *  o limiar publicado pelo Google (`https://web.dev/articles/ttfb`, atualizada em 18/11/2025).
 *  Entre 600 e 800ms a tela REPROVAVA o que a fonte oficial aprova — falso-negativo, não rigor.
 *
 *  `ideal: 300` FICA e continua SEM FONTE: o Google não publica um "ideal" de TTFB. É alvo
 *  editorial nosso, e está registrado como tal em `lib/gsc-delta.mjs#EDITORIAIS`. Ele aparece AO
 *  LADO do limite e nunca no lugar dele (FR-007), então não emite veredito — por isso pode
 *  conviver com um limiar que tem fonte sem contaminá-lo.
 *  @type {readonly Vital[]} */
export const VITAIS = [
  ...
  { id: "ttfb", chaveCrux: "experimental_time_to_first_byte", limite: 800, ideal: 300, unidade: "ms" },
];

/** FR-014 — o escopo. Projeto fora da lista não gasta chamada e mantém o "sem coletor nesta
 *  requisição" que a ficha já exibe hoje.
 *
 *  052/FR-006/research D9 — o Sirius entra. Lista separada de `SLUGS_DE_BUSCA` (`lib/projects.ts`)
 *  de propósito: a CrUX é outra fonte, com outra quota. Efeito colateral desejado: `lib/ficha-
 *  dados.ts:93` passa a ler o campo de `/okr/sirius` também. */
export const SLUGS_DE_CAMPO = ["atma", "sirius"];
...
/**
 * 042 — qual ORIGEM responde por um vital quando o projeto declara mais de uma (troca de domínio).
 *
 * Medido na atma em 21/09/2026: `usealigner.com` (o card) dá 404 na CrUX, e
 * `atma.roilabs.com.br` (o `dominioAnterior`) devolve LCP, CLS e TTFB na janela 23/08→19/09 — sem
 * INP. Perguntar só pela origem do card publicaria "sem amostra" sobre um site que tem 28 dias de
 * campo; perguntar só pela antiga publicaria o domínio velho como se fosse o atual. Por isso a
 * função recebe AS DUAS, na ordem declarada (a atual primeiro), e a tela nomeia a que respondeu.
 *
 * Ausência na ordem da causa mais específica: uma origem respondeu SEM este vital (`parcial`, o
 * INP de site com pouca interação) › alguma falhou (não dá para afirmar falta de amostra) › todas
 * sem chave › todas sem amostra. `falhou` nunca se disfarça de `sem-amostra` (FR-003).
 * @param {{alvo: Alvo, leitura: LeituraDaFonte}[]} leituras na ordem de `hostsDeclarados()`
 * @param {string} id `lcp` | `inp` | `cls` | `ttfb`
 * @returns {{estado:"medido", medida:Medida, alvo:Alvo} | {estado:"parcial", alvo:Alvo} | {estado:"falhou", erro:string} | {estado:"sem-chave"} | {estado:"sem-amostra"}}
 */
export function vitalPorOrigem(leituras, id) {
  const vital = VITAIS.find((v) => v.id === id);
  let parcial = null;
  let falha = null;
  for (const { alvo, leitura } of leituras) {
    if (leitura?.estado === "record") {
      const medida = medirRecord(leitura.record).get(vital.chaveCrux);
      if (medida) return { estado: "medido", medida, alvo };
      parcial ??= alvo;
    } else if (leitura?.estado === "falhou") falha ??= leitura.erro ?? "leitura de campo indisponível";
  }
  if (parcial) return { estado: "parcial", alvo: parcial };
  if (falha !== null) return { estado: "falhou", erro: falha };
  return leituras.length && leituras.every((l) => l.leitura?.estado === "sem-chave") ? { estado: "sem-chave" } : { estado: "sem-amostra" };
}
...
export function passRate(leiturasPorUrl, consultadas) {
  ...
  let parciais = 0;
  let falharam = 0;
  /** 028 — o veredito POR URL, apurado na mesma passagem que os agregados.
   *
   *  A tela desenha uma célula por URL consultada, e a célula não pode ter uma régua própria: a
   *  memória deste projeto registra seis vezes o mesmo defeito — guarda reimplementada no chamador
   *  divergindo da função compartilhada na primeira variante nova. Aqui a fração e as células saem
   *  do MESMO laço, então não existe o estado em que a barra diz uma coisa e a contagem outra. */
  const porUrl = [];
  for (const [url, leitura] of leiturasPorUrl.entries()) {
    if (leitura?.estado === "falhou") {
      falharam += 1;
      porUrl.push({ url, estado: "falhou" });
      continue;
    }
    if (leitura?.estado !== "record") {
      // `sem-amostra` é o protocolo da fonte para "não tem visita suficiente" (404) e `sem-chave`
      // é o hub desligado. Colapsar os dois num "sem dado" é a confusão que a FR-003 proíbe.
      porUrl.push({ url, estado: leitura?.estado === "sem-amostra" ? "sem-amostra" : leitura?.estado === "sem-chave" ? "sem-chave" : "nao-lida" });
      continue;
    }
    const medidas = medirRecord(leitura.record);
    ...
    if (tres.some((m) => !m)) {
      parciais += 1;
      porUrl.push({ url, estado: "parcial" });
      continue;
    }
    comDado += 1;
    const passou = tres.every((m) => m.veredito === "dentro");
    if (passou) passam += 1;
    porUrl.push({ url, estado: passou ? "passa" : "reprova" });
  }
  if (comDado < 2) {
  ...
  return { consultadas, comDado, passam, parciais, falharam, porUrl, fracao: passam / comDado, motivo: null };
}
