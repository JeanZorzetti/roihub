...
export function passRate(leiturasPorUrl, consultadas) {
  ...
  if (comDado < 2) {
    return {
      ...
      passam,
      fracao: null,
      // O motivo nomeia o número que EXISTE, não só a ausência (SC-006): quem lê precisa entender
      // que é característica do tráfego do site, não defeito do hub.
      motivo:
        `Pass Rate por URL não é apurável: das ${consultadas} URLs consultadas, ${comDado} ${comDado === 1 ? "tem" : "têm"} dado de campo. ` +
        "A fonte só reporta URL com visitas suficientes, e a fração precisa de pelo menos duas URLs medidas — uma amostra de uma nunca é 100%.",
    };
  }
  return { consultadas, comDado, passam, fracao: passam / comDado, motivo: null };
}
