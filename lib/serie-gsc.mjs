...
export function somarSeriesPorHost(series) {
  ...
  for (const s of series ?? []) {
    for (const d of s?.days ?? []) {
      if (!d?.date) continue;
      const acc = porDia.get(d.date) ?? { date: d.date, clicks: 0, impressions: 0, somaPos: 0, impPos: 0, votos: 0, posUnica: null };
  ...
  return [...porDia.values()]
    ...
    .map(({ somaPos, impPos, votos, posUnica, ...d }) => ({
      ...d,
      // Voto único devolve a posição do Google como veio — a mesma régua de `mesclarPorCaminho`.
      // (3,9 × 1146) ÷ 1146 dá 3,8999999999999995, e medido em 20.000 casos 1.574 saíam alterados:
      // a leitura ao vivo passou a somar todo projeto, e os de UM host não podem mudar na 16ª casa.
      ...
      position: votos === 1 ? posUnica : impPos > 0 ? somaPos / impPos : null,
    }));
}
