 ...
 * contato OU deal criado 5 min ou mais depois da conta. Os contatos e deals de exemplo nascem no mesmo minuto
 ...
 * @param {{id:string, livemode:boolean, pago:boolean, valor:number, conta:string|null, data:string, reembolsada:boolean}[]} sessoes
    ...
    const motivo = !s.livemode
      ...
      : !s.pago
        ...
        : !(s.valor > 0)
          ? "valor zero"
          : !s.conta
          ? "sem conta do Sirius (a conta Stripe é compartilhada com outros produtos)"
          : !conta
...
export function pagantes({ declarados, stripe, contasPorId, janela }) {
  ...
  // A cadeia divide pagantes por ativadas como se uma estivesse dentro da outra. Com ativação só por
  // contato, uma pagante (33 deals, nenhum contato) ficava fora — foi o que levou a "contato OU deal" em
  // 22/09/2026. Se voltar a acontecer, a tela nomeia a conta em vez de deixar a taxa parecer exata.
