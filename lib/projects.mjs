 ...
 */
/**
 * 052/research D2 — os projetos com corrida de busca (e, pela mesma lista, com mapa): card
 * curado, com `url`, na ordem de `slugs`. A guarda mora aqui, na função que corridas e mapa
 * chamam, não em cada chamador: posta só numa página, a corrida seguiria medindo um repo sem
 * card enquanto o mapa desse 404, e as duas listas divergiriam.
 * @param {any[]} projetos já mesclados por `mergeProjects`
 * @param {string[]} slugs `SLUGS_DE_BUSCA` — a ordem do resultado é a ordem desta lista
 * @returns {any[]}
 */
export function deBusca(projetos, slugs) {
  return slugs.map((s) => projetos.find((p) => p.slug === s && p.curated && p.url)).filter(Boolean);
}
