...
import { PERFIS, FAMILIAS } from "./okr.mjs";
...
export function resolverTicket(ticketApurado, meta) {
  if (ehApurado(ticketApurado)) {
    // Os dois denominadores entram no rótulo (018/FR-021a). Desde a 051/FR-012b a média é por
    // PESSOA, pelo último orçamento de cada uma, e os documentos enviados vêm junto — na atma de
    // 21/09 são 13 documentos para 7 pessoas. `docs`/`pessoas` vêm de `ticketDeOrcamentos()`;
    // quando faltam (chamador antigo, teste), o rótulo volta ao texto genérico em vez de mentir.
    const { docs, pessoas, orfaos } = /** @type {{docs?:number, pessoas?:number, orfaos?:number}} */ (ticketApurado);
    // O órfão conta como destinatário aqui e fica fora de vivos/perdidos no valor em risco
    // (FR-015a) — a decomposição sai junto para a conta fechar na tela, não na cabeça de quem lê.
    const quem =
      orfaos
        ? `${pessoas} pessoa${pessoas === 1 ? "" : "s"} (${pessoas - orfaos} com lead + ${orfaos} sem)`
        : `${pessoas} pessoa${pessoas === 1 ? "" : "s"}`;
    const denominador =
      docs != null && pessoas != null
        ? `média do último orçamento de ${quem}, entre ${docs} orçamento${docs === 1 ? "" : "s"} enviado${docs === 1 ? "" : "s"}`
        : "média de orçamentos";
 ...
 * @param {Record<string, {valor:number|string, fonte?:string}|{naoApurado:string, fonte?:string, rotuloBuraco?:"falhou-agora"}>} disponiveis
...
export function montarN5(familia, disponiveis, perfil) {
  ...
  return MEDIDORES[familia].filter((id) => medidorCabeNoPerfil(perfil, id)).map((id) => {
    ...
    if (celula) {
      // 023: a célula de vital chega FORMATADA ("2,4 s", "0,08") e `ehApurado()` exige
      // `number` — é ele que protege a aritmética de `razao()` de somar string. O envelope
      // do valor já formatado é montado aqui, sem afrouxar a guarda de que o resto depende.
      const fonte = celula.fonte ?? "coleta desta requisição";
      const c =
        typeof celula.valor === "string"
          ? { estado: "apurado", valor: celula.valor, rotulo: id, fonte }
          : estadoDeApurado(celula, fonte, id);
      // 023/D6: `estadoDeApurado()` DESCARTA `rotuloBuraco`, e sem ele a tela lê "não apurado"
      // onde a fonte falhou AGORA — a distinção da FR-003 morreria na borda. Repassado pelo
      // campo, nunca inferido por regex sobre o texto do motivo: acoplamento por string que
      // qualquer revisão de UX writing quebra em silêncio.
      if (c.estado === "nao-apurado" && celula.rotuloBuraco !== undefined) c.rotuloBuraco = celula.rotuloBuraco;
      return { id, nome: id, familia, celula: c };
    }
 ...
 *   disponiveisN5: Record<string, {valor:number|string, fonte?:string}|{naoApurado:string, fonte?:string, rotuloBuraco?:"falhou-agora"}>,
...
export function montarNiveis(entrada) {
  ...
  if (semPerfil) {
    ...
    // 023/D11: Entrega é PRÉ-CONDIÇÃO, não degrau de funil — `escolherFamilia()` nunca devolve
    // "D2" (nenhum perfil declara essa família e todo fallback dá D1/D3/D4), então as células de
    // vitais eram computadas e descartadas, e `n5:lcp` saía `chave-invalida` desde a 011. A
    // condição é HAVER MEDIDA, não o slug: este módulo é puro e não conhece `SLUGS_DE_CAMPO`, e
    // exibir D2 incondicionalmente poria 8 linhas permanentes de "não apurado" nas outras 34
    // fichas — o ruído com cara de pendência que `medidorCabeNoPerfil()` já rejeitou (FR-002a).
    const disponiveis = disponiveisN5 ?? {};
    const temEntrega = MEDIDORES.D2.some((id) => id in disponiveis);
    const medidores = [
      ...montarN5(familia, disponiveis, ficha.perfil),
      ...(temEntrega && familia !== "D2" ? montarN5("D2", disponiveis, ficha.perfil) : []),
    ];
    // FR-002b: com dois grupos de medidores na tela, quem lê precisa saber qual é o gargalo da
    // cadeia e por que o outro está ali.
    const notaN5 =
      temEntrega && familia && familia !== "D2"
        ? `O gargalo da cadeia é ${FAMILIAS[familia] ?? familia} — ${motivoFamilia}. ${FAMILIAS.D2} aparece junto porque é pré-condição de todo o resto: se a página não chega inteira, nenhum degrau do funil é lido direito.`
        : undefined;
    ...
    n5 = { id: "N5", ...TITULOS_NIVEL.N5, celulas: medidores.map((m) => m.celula), familia, motivoFamilia, ...(notaN5 ? { nota: notaN5 } : {}) };
    espacosKr = {
