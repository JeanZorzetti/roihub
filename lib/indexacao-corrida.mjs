...
/**
 * As duas razões do board sobre as contagens — de `agregar()` ou da linha já gravada em
 * `hub_indexacao`.
 *
 ...
 * `null` — "não apurado", nunca 0%. `rejeicao` é o mesmo denominador com os dois baldes que o board
 * nomeia ("Rastreada, mas não indexada" e "Descoberta, mas não indexada").
 *
 * 043 — mesma razão de `coberturaRich`: a divisão vivia escrita na corrida E em
 * /okr/[slug]/aquisicao, e /gsc/mapa seria a terceira cópia.
 *
 * @param {{inspecionadas: number, falhas: number, indexadas: number, rastreadasNaoIndexadas: number, descobertasNaoIndexadas: number}} c
 * @returns {{base: number, taxa: number|null, rejeicao: number|null}}
 */
export function taxasDeIndexacao({ inspecionadas, falhas, indexadas, rastreadasNaoIndexadas, descobertasNaoIndexadas }) {
  const base = inspecionadas - falhas;
  return {
    base,
    taxa: base > 0 ? indexadas / base : null,
    rejeicao: base > 0 ? (rastreadasNaoIndexadas + descobertasNaoIndexadas) / base : null,
  };
}

/**
 * As cinco contagens mais as duas taxas (`taxasDeIndexacao`).
 *
...
export function agregar(linhas) {
  ...
  const { taxa, rejeicao } = taxasDeIndexacao({ ...c, inspecionadas: linhas.length });
