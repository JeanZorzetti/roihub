...
import { pipelineDe, celulaDeLeads } from "@/lib/okr.mjs";
 ...
 */
export const FONTES_PROPRIAS: Record<string, { env: string; tabela: string; sql: string }> = {
  atma: {
...
export async function coletarDoProjeto(
...
): Promise<{ cliques: Celula; leads: Celula; vendas: Celula; impressoes: Celula }> {
  ...
  return { cliques, leads: leadsCel, vendas, impressoes };
}
