 ...
 */
export function activeIndexRatio(linhas, indexadas) {
  if (!Number.isFinite(indexadas) || indexadas <= 0) return null;
  return urlsComImpressao(linhas) / indexadas;
}
 ...
 *  `ehMarca` só é REPASSADO — filtrar aqui daria uma segunda régua de marca ao lado da de
 *  `canibalizacao()`, e as duas divergiriam na primeira variante nova.
 *  @param {LinhaBusca[]} linhas
 *  @param {((consulta: string) => boolean)|null} [ehMarca] */
export function kpisDeBusca(linhas, ehMarca = null) {
  return {
