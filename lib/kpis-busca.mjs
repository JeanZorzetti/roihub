...
/**
 * 050 — ESTIMATIVA da cobertura do TAM de busca: impressões da janela ÷ demanda estimada, termo a
 * termo, sobre o inventário congelado. O GSC não informa volume de mercado. Mas impressão só existe
 * onde houve busca, então o maior número de impressões que um termo teve numa janela de 28 dias é um
 * PISO do volume dele (`data/demanda-estimada.json`, `scripts/estimar-demanda.mjs`). Com denominador
 * abaixo do real, a razão é TETO.
 *
 * O numerador é impressão, não "termo no Top 20 ponderado pelo volume", porque a posição do GSC é a
 * média só das buscas em que o site apareceu. Medido na Atma em 21/09/2026: «invisalign» na posição
 * 3,6 com 24 impressões contra 6.934 no pico. A versão por posição dá 72,9%, dentro da meta do board,
 * enquanto a versão por impressão dá 15,8%.
 *
 * O denominador de cada termo é `max(pico, hoje)`. Impressão de hoje acima do pico também prova
 * volume, e assim a razão nunca passa de 100%. Como esse denominador só cresce, ele não tem como
 * inflar a cobertura.
 *
 * @param {{termo: string, impressoes: number}[]} linhas leitura na dimensão `query`
 * @param {Record<string, number>|null|undefined} demanda pico de impressões por termo, congelado
 * @returns {{fracao:number, impressoes:number, demanda:number, termos:number,
 *            buracos:{termo:string, pico:number, hoje:number}[]} | null}
 */
export function coberturaDaDemanda(linhas, demanda) {
  const termos = Object.keys(demanda ?? {});
  if (!termos.length) return null;
  const hoje = new Map();
  for (const l of linhas ?? []) if (l.termo in demanda) hoje.set(l.termo, (hoje.get(l.termo) ?? 0) + (l.impressoes ?? 0));
  const porTermo = termos.map((termo) => {
    const h = hoje.get(termo) ?? 0;
    return { termo, pico: Math.max(demanda[termo], h), hoje: h };
  });
  const impressoes = porTermo.reduce((a, t) => a + t.hoje, 0);
  const total = porTermo.reduce((a, t) => a + t.pico, 0);
  if (total <= 0) return null;
  const buracos = porTermo.sort((a, b) => b.pico - b.hoje - (a.pico - a.hoje));
  return { fracao: impressoes / total, impressoes, demanda: total, termos: termos.length, buracos };
}
