...
export function penetracaoNoTop3(linhas, inventario) {
  ...
  const noTop3 = naJanela.filter((l) => typeof l.posicao === "number" && l.posicao <= 3 && l.posicao >= 1).length;
  const total = inventario.total ?? inventario.termos.length;
  const cobertura = new Set(naJanela.map((l) => l.termo)).size;
  return { fracao: noTop3 / total, noTop3, total, cobertura, piso: cobertura < total };
}
