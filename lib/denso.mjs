...
async function ollamaEmbed(entradas) {
...
// Embeda o que faltar e grava. nomic distingue documento de consulta pelo prefixo — trocar os
// dois derruba a similaridade sem erro nenhum.
export async function embedar(textos, { prefixo = "search_document: ", lote = 32 } = {}) {
...
const cosseno = (a, b) => {
  ...
  return d; // nomic devolve vetor normalizado: produto interno já é o cosseno
};
...
export async function buscarDenso(indice, consulta, k = 10) {
  const [q] = await embedar([consulta], { prefixo: "search_query: " });
