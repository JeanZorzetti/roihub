 ...
 *
 * `ctr` carrega a JANELA que o produziu (019, FR-030): sem ela não há como provar que o CTR e a
 * cadeia mediram o mesmo período, e a camada de impressões não entra.
 *
 * @param {{ficha:{perfil?:string|null, marcos:{chave:string,nome:string,celula:Celula}[]},
 *          projecao:{n1Janela:Celula},
 *          ctr?:{valor:number, impressoes?:Celula, janela?:{inicio:string,fim:string}},
 *          janelaConversao?:{inicio:string,fim:string}}} entrada
 */
export function montarArvore({ ficha, projecao, ctr, janelaConversao }) {
  ...
  //
  // 019/FR-031: a guarda deixou de ser `marcos[0]?.chave === "visitante"` e passou a ser
  // COINCIDÊNCIA DE JANELA. Nome de marco não prova que as janelas batem — foi a trava certa na
  // 018 porque era a única disponível, e ela recusava por engano a cadeia D nova, cujo `ctr`
  // fatiado pela época mede exatamente o mesmo período que a cadeia. CONTÉM, não iguala: a série
  // do GSC é maior que a época e é fatiada por ela.
  if (!parou && i === 0 && divide(ctr)) {
    const cobre =
      !!ctr.janela &&
      !!janelaConversao &&
    ...
    if (cobre) {
      ...
      camadas.push({
        ...
        divisor: {
          ...
          lo: ctr.valor,
          hi: ctr.valor,
          fonte: `CTR apurado da mesma série do Search Console, ${ctr.janela.inicio} → ${ctr.janela.fim}`,
          atravessa: [],
        },
      });
    } else if (ctr.janela && janelaConversao) {
      // FR-033: quando NÃO contém, a árvore PARA e NOMEIA o que faltou. Nunca compõe períodos
      // diferentes (018/FR-007 continua íntegra) — e nunca some em silêncio, que é indistinguível
      // de não ter sido calculada. Sem `ctr.janela` não há o que nomear: aí é o caso de sempre
      // (projeto sem época, FR-032), e a árvore simplesmente termina na cadeia.
      parou = {
        nome: "impressões",
        motivo: `a série do Search Console cobre ${ctr.janela.inicio} → ${ctr.janela.fim}, e a janela desta cadeia é ${janelaConversao.inicio} → ${janelaConversao.fim} — compor períodos diferentes seria inventar o dado`,
      };
    }
  }
