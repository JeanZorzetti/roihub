...
import { benchmark as ctrEsperado, PISO_IMPRESSOES_VEREDITO } from "./kpis-busca.mjs";
...
export const MEDIDO_POR = {
  ctrPorPosicao: "lib/kpis-busca.mjs#porUrl+benchmark",
  // `penetracaoTop3` SAIU em 19/09/2026 e a ausência é o conserto, não uma lacuna. Ela apontava
  // para `impressoesNoTop3`, que soma IMPRESSÕES; a folha conta TERMOS ("termos com posição ≤ 3 ÷
  // total monitorado"). São grandezas diferentes com metas diferentes — e a prova estava no próprio
  // mapa: `impressoesTop3` credita o MESMO coletor com meta de 40% a 50%, contra os 20% a 30% desta.
  // Quem abrisse a folha lia "medido" e comparava um percentual de impressões contra uma meta de
  // inventário de palavras-chave. O hub não mantém inventário monitorado, então o coletor não
  // existe — e "nenhum coletor" é a frase verdadeira.
  ...
  ctrGap: "lib/kpis-busca.mjs#ctrGap",
  conformidadeUrls: "lib/kpis-busca.mjs#ctrGap.fracao",
  schema: "lib/grafo.mjs#taxaCobertura",
  larguraTitulo: "lib/grafo.mjs#taxaIntegridadeDoTitulo",
  termoNoTitulo: "lib/pagina.mjs#posicaoDoTermo + lib/grafo.mjs#TERMO_ATE",
  intencao: "lib/grafo.mjs#taxaAlinhamento",
  lcp: "lib/crux.mjs#celulasDeVitais",
  inp: "lib/crux.mjs#celulasDeVitais",
  cls: "lib/crux.mjs#celulasDeVitais",
  urlsBoas: "lib/crux.mjs#passRate",
  ttfb: "lib/crux.mjs#VITAIS",
 ...
 *
 * `noTop20` e `activeIndexRatio` não carregam a flag no retorno e herdam a MESMA limitação: as duas
 * contam sobre as linhas que a dimensão `query` devolveu.
 *
...
const E_A_JANELA = " — e a janela de 28 dias do board é recortada pela borda, não pela função pura";
export const RESSALVA_DO_COLETOR = {
  ...
  activeIndexRatio: DIMENSAO_QUERY + E_A_JANELA,
};
...
export const CATALOGO = {
  ...
  ctrPorPosicao: {
    ...
    ramo: "clique",
    balizador: { tipo: "regua", ...CTR_PISO, media: null, elite: null, unidade: "ctr-por-posicao", base: "impressoes" },
  },
  penetracaoTop3: {
  ...
  ctrGap: {
    ...
    ramo: "ctr",
    balizador: { tipo: "regua", ...CTR_PISO, unidade: "cliques", base: "impressoes" },
  },
  conformidadeUrls: {
...
/**
 * Os cliques que a SERP ofereceu e o snippet não pegou, numa URL abaixo do benchmark da posição.
 *
 * `impressões × (benchmark − ctr)` é aritmética de UM degrau: carrega a barra de erro do CTR
 * sozinho, nada é multiplicado por nada de outro degrau, e nenhum coeficiente é inventado. É a
 * ÚNICA conversão para cliques deste arquivo — as outras réguas (vitais, largura de título) não
 * convertem sem um fator que ninguém publicou, e por isso saem em `pp` ou na unidade delas.
 *
 * @param {{impressoes:number, ctr:number, benchmark:number}} u
 * @returns {number|null} cliques não capturados, ou null quando a URL já cobre o benchmark
 */
export function cliquesNaoCapturados(u) {
  ...
  // Zero impressão devolve `null`, NUNCA `0`. Mesma regra de `ctr()` em `kpis-busca.mjs`: exibir
  // zero para uma página que ninguém viu inventa desempenho onde não houve medição — e aqui seria
  // pior, porque `0` entraria na fila de trabalho parecendo "já cobre o benchmark".
 ...
 * @param {{base?:number|null, exigePiso?:boolean, falha?:string|null}} [ctx]
 * @returns {{selo: Selo, palavra?: string}}
 */
export function seloDaMedida(celula, { base = null, exigePiso = false, falha = null } = {}) {
  if (falha) return { selo: "cega", palavra: falha };
  if (!ehApurado(celula)) return { selo: "sem" };
  if (exigePiso) {
    if (!Number.isFinite(base)) throw new Error("leitura com piso de amostra exige `base` (impressões da janela)");
    // 026 — abaixo do piso o número CONTINUA na tela, com a base ao lado: o que sai é a régua.
    if (base < PISO_IMPRESSOES_VEREDITO) return { selo: "piso", palavra: `piso: ${base} impressões na janela` };
  }
  return { selo: "dado" };
}
...
/** A régua exige piso de amostra? Consulta única, para a tela não repetir a condição em 17 pontos. */
export const exigePiso = (chave) => CATALOGO[chave]?.balizador?.base === "impressoes";
 ...
 * número visível.
 *
 * A condição de validade entra aqui pelo mesmo motivo: essas duas são as únicas com
 * `base: "impressoes"`, ou seja, as únicas cuja régua só vale acima de `PISO_IMPRESSOES_VEREDITO` —
 * e esse piso não aparecia em pixel nenhum das duas telas. Régua publicada sem a condição sob a qual
 * vale é régua que aprova uma amostra de 26 impressões.
 *
 * @param {string} chave
...
export function limiarEmTexto(chave) {
  ...
  const sufixo = exigePiso(chave) ? ", e só vale acima de " + PISO_IMPRESSOES_VEREDITO + " impressões na janela" : "";
...
export function linha(chave, celula, medida = {}) {
  ...
  const r = regua(chave);
  const { selo, palavra } = seloDaMedida(celula, { base, exigePiso: exigePiso(chave), falha });
...
export { ctrEsperado, PISO_IMPRESSOES_VEREDITO };
