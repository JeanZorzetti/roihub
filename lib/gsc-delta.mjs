...
import { apurado, ehApurado } from "./funil.mjs";
...
export const MEDIDO_POR = {
  ...
  ttfb: CAMPO,
  // 043 — as duas razões creditadas à função que ABRE o nó no mapa (`taxasDeIndexacao`, a mesma da
  // corrida e de /okr/[slug]/aquisicao), mais a que decide o balde de cada URL. `rejeicaoRastreio`
  // era `semColetor` desde 19/09 com a razão calculada em `agregar()` desde a 022 e testada por
  // nome ("a rejeição de rastreio soma as duas classes") — o erro que este mapa nasceu para travar.
  indexacaoLimpa:
    "lib/indexacao-corrida.mjs#taxasDeIndexacao (indexadas ÷ inspecionadas sem falha) + lib/indexacao.mjs#estaIndexada (o que conta como indexada)",
  rejeicaoRastreio:
    "lib/indexacao-corrida.mjs#taxasDeIndexacao (rastreadas + descobertas ÷ inspecionadas sem falha) + lib/indexacao-corrida.mjs#classificar (os dois baldes)",
  profundidadeClique: "lib/grafo.mjs#profundidades",
  frescor: "lib/grafo.mjs#cadencia",
  // 044 — DOIS coletores, e o primeiro é o que ABRE o nó no mapa: a meta do board conta PÁGINAS
  // ("0 páginas competindo pela mesma palavra-chave primária") e `canibalizacao` conta CONSULTAS —
  // na Atma, 73 consultas saíam de 5 páginas. Creditar só a segunda repetiria o defeito que a 034
  // corrigiu em `penetracaoTop3`.
  canibalizacao:
    "lib/kpis-busca.mjs#canibalizacaoPorPagina (páginas com a palavra-chave primária disputada) + lib/kpis-busca.mjs#canibalizacao (consultas servidas por 2+ URLs)",
  linksInternos: "lib/grafo.mjs#densidades",
  // 045 — a proporção abre o nó e os meses respondem à meta do board, que é volume MENSAL crescente.
  buscasDeMarca: "lib/marca.mjs#razaoDeMarca (marca ÷ total do corte de país) + lib/marca.mjs#mesesFechados (o volume mensal que a meta pede)",
...
/**
 * 051 — as três classes de folha. O 4DX separa preditiva de resultado; aqui são TRÊS porque metade do
 * board não é nenhuma das duas: é limiar. Tratar um LCP de 1,8 s, que já passa, como alavanca manda
 * trabalhar onde não rende — e chamar título de "preditivo" afirma um efeito que ninguém testou.
 *
 * O indicador preditivo do 4DX de verdade é contagem de AÇÃO por semana, e ele não está no mapa:
 * `acaoSemanal` só o nomeia (decisão do dono, 21/09/2026 — o hub não tem fonte para contar).
 */
export const CLASSES = {
  resultado: {
    rotulo: "resultado",
    nota: "Resultado: é a reação do Google, com atraso de dias a meses. Mede o efeito; não se move direto.",
  },
  alavanca: {
    rotulo: "alavanca",
    nota: "Alavanca: a equipe move esta folha direto. Que isso aumente clique é hipótese não testada — um site só, e uma série cortada pela desindexação e pela troca de domínio.",
  },
  higiene: {
    rotulo: "higiene · limiar",
    nota: "Higiene: é limiar. Abaixo dele atrapalha; passado ele, não se espera ganho de melhorar mais.",
  },
};
 ...
 *
 * 051 — `classe` mora AQUI, e não num mapa ao lado: é natureza da folha como o `balizador`, e uma
 * segunda lista de folhas é o defeito que reconstruiu o board três vezes. `acaoSemanal` só existe na
 * alavanca (ver `CLASSES`).
 *
 * @type {Record<string, {nome: string, ramo: string, classe?: keyof typeof CLASSES, acaoSemanal?: string, balizador: Regua|Recusa|SemColetor|Norma|Procedimento}>}
 */
export const CATALOGO = {
  ...
  schema: {
    ...
    ramo: "ctr",
    classe: "alavanca",
    acaoSemanal: "páginas com schema corrigido por semana",
  ...
  larguraTitulo: {
    ...
    ramo: "ctr",
    classe: "alavanca",
    acaoSemanal: "títulos encurtados por semana",
    balizador: {
      tipo: "regua",
  ...
  termoNoTitulo: {
    ...
    ramo: "ctr",
    classe: "alavanca",
    acaoSemanal: "títulos reescritos por semana",
    balizador: {
      tipo: "recusa",
  ...
  intencao: {
    ...
    ramo: "ctr",
    classe: "alavanca",
    acaoSemanal: "títulos com modificador por semana",
    balizador: { tipo: "norma", motivo: "tem ou não tem o modificador: sem faixa de mercado por trás" },
  },

  // ---------- ramo POSIÇÃO MÉDIA ----------
  lcp: { nome: "LCP", ramo: "posicao", classe: "higiene", balizador: { tipo: "regua", ...WEBDEV, limite: 2500, unidade: "ms" } },
  inp: { nome: "INP", ramo: "posicao", classe: "higiene", balizador: { tipo: "regua", ...WEBDEV, limite: 200, unidade: "ms" } },
  cls: { nome: "CLS", ramo: "posicao", classe: "higiene", balizador: { tipo: "regua", ...WEBDEV, limite: 0.1, unidade: null } },
  urlsBoas: {
  ...
  rejeicaoRastreio: {
    ...
    balizador: {
      tipo: "recusa",
      motivo:
        "a faixa não é publicada: os < 5% são do board, e o Google não divulga quanto do inventário costuma ficar em 'Descoberta' ou 'Rastreada, mas não indexada' — o número existe (URL Inspection, corrida diária), a régua não",
    },
  },
  profundidadeClique: {
  ...
  coberturaSemantica: {
    ...
    ramo: "posicao",
    classe: "alavanca",
    acaoSemanal: "subtemas cobertos por semana",
  ...
  frescor: {
    ...
    ramo: "posicao",
    classe: "alavanca",
    acaoSemanal: "páginas revisadas por semana",
    balizador: {
      tipo: "recusa",
      motivo:
  ...
  canibalizacao: {
    ...
    ramo: "posicao",
    classe: "alavanca",
    acaoSemanal: "páginas consolidadas por semana",
  ...
  linksInternos: {
    ...
    ramo: "posicao",
    classe: "alavanca",
    acaoSemanal: "links contextuais por semana",
    balizador: {
      tipo: "recusa",
      motivo:
...
const SOBREPOSICAO =
  "as 6 faixas de “Posição no Google” contam os mesmos cliques do CTR Gap por outro corte (a posição média da página) e ficam fora da fila: entrariam duas vezes";

/**
 * 051/US3 — a fila 80/20 do mapa: o que rende mais primeiro, DENTRO de cada moeda. Monta itens com
 * `linha()` e ordena com `ordenar()`, que existiam com teste desde 19/09 e nenhuma tela chamava.
 *
 * Em cliques, um item por URL decidida abaixo da régua, e não um por folha: "o que fazer primeiro" é
 * uma página. Em 21/09 o primeiro era o post de preço, 153 cliques por 28 dias, com 93,9% do tráfego
 * decidível — a concentração que o mapa não mostrava. Em pp, a largura de título (a fração acima de
 * 580px). Vitais só entram acima do limite; hoje os três com amostra passam.
 *
 * Tudo o que fica de fora sai CONTADO e com o motivo: fila que some com 24 folhas em silêncio lê como
 * "o resto está bem", e a maioria delas nem tem régua.
 *
 * @param {{
 *   abaixo: {url:string, impressoes:number, ctr:number|null, regua:number, posicao:number}[]|null,
 *   largura: {fracao:number, avaliadas:number, largos:unknown[]}|null,
 *   vitais: {chave:string, valor:number|null, limite:number, falhou?:boolean}[],
 * }} leituras  `null` = a página não conseguiu ler a fonte
 */
export function filaDoMapa({ abaixo, largura, vitais }) {
  const itens = [];
  const fora = { recusa: 0, norma: 0, semColetor: 0, procedimento: 0, dentroDaRegua: [], semAmostra: [], semLeitura: [] };
  for (const { balizador } of Object.values(CATALOGO)) if (balizador.tipo !== "regua") fora[balizador.tipo]++;

  if (!abaixo) fora.semLeitura.push("ctrGap");
  else
    for (const u of abaixo) {
      const delta = cliquesNaoCapturados({ impressoes: u.impressoes, ctr: u.ctr, benchmark: u.regua });
      if (delta === null) continue;
      itens.push({ ...linha("ctrGap", apurado(u.ctr), { delta, moeda: "cliques", base: u.impressoes }), alvo: u.url, detalhe: { ctr: u.ctr, regua: u.regua, posicao: u.posicao } });
    }

  if (!largura) fora.semLeitura.push("larguraTitulo");
  else if (!largura.largos.length) fora.dentroDaRegua.push("larguraTitulo");
  else
    itens.push({
      ...linha("larguraTitulo", apurado(largura.fracao), { delta: (largura.largos.length / largura.avaliadas) * 100, moeda: "pp", base: largura.avaliadas }),
      alvo: null,
      detalhe: { acima: largura.largos.length, avaliadas: largura.avaliadas },
    });

  for (const v of vitais) {
    // Leitura que falhou (ou sem chave) não é site sem amostra: um pede esperar, o outro consertar.
    if (v.falhou) fora.semLeitura.push(v.chave);
    else if (v.valor === null || v.valor === undefined) fora.semAmostra.push(v.chave);
    else if (v.valor <= v.limite) fora.dentroDaRegua.push(v.chave);
    // CLS não tem unidade: a moeda é o próprio nome, e ela nunca divide bloco com milissegundo.
    else itens.push({ ...linha(v.chave, apurado(v.valor), { delta: v.valor - v.limite, moeda: v.chave === "cls" ? "cls" : "ms" }), alvo: null, detalhe: { limite: v.limite } });
  }

  return { porMoeda: ordenar(itens).porMoeda, fora, sobreposicao: SOBREPOSICAO };
}
