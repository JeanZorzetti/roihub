...
export const REGUA = {
  ...
  D: {
    ...
    //
    // `contatado→orcamento` e `orcamento→tratamento`: sem linha de propósito. Ninguém publica
    // esses degraus isolados — o primeiro não existe na literatura, o segundo (o que sobrou de
    // `aceito→tratamento`) vive confundido com o próprio aceite. Estimar aqui seria inventar o
    // número mais decisivo da cadeia.
  },
  A: {
    "visitante→signup": {
      media: [0.02, 0.05],
...
export function faixaDoSpan(perfil, chaveDe, chavePara) {
  ...
  return REGUA[perfil]?.[`${chaveDe}→${chavePara}`] ?? null;
}
