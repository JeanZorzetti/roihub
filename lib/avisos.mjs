...
// Escopo fechado pelo dono em 15/09/2026 (026) e 16/09/2026 (027): os outros leads do hub não
// avisam. Atma e ROI Labs têm pipeline aqui, mas não mandam lead ao hub; avisam por /api/avisos/evento.
const PIPELINES_COM_AVISO = ["sirius", "estetiacrm", "verticemarketing"];
...
// Projetos de /api/avisos/evento (027). Tabela local, e não pipelines.json: `coopluz` nem existe lá
// (o lead dela vive no autogestor). A base do link é fixa pelo mesmo motivo do PAINEL: quem tem o
// segredo escolhe só o caminho, nunca o domínio.
const PROJETOS = {
  atma: { nome: "Atma", base: "https://atmaadmin.roilabs.com.br" },
  roilabs: { nome: "ROI Labs", base: "https://app.roilabs.com.br" },
  coopluz: { nome: "Coopluz", base: "https://admin.autogestor.roilabs.com.br" },
};

// A Bot API limita o texto a 4.096 caracteres depois do parse das tags; 3.500 + título + link cabe
// com folga. O resumo semanal do ROI Labs é o único alerta que chega perto.
const MAX_TEXTO = 3500;

/**
 * Normaliza e valida o corpo de `POST /api/avisos/evento`. O texto vem montado pela origem; o hub
 * só corta, escapa e põe o link.
 *
 * @returns {{ ok: true, aviso: { projeto: string, titulo: string, texto: string, caminho: string,
 *   acao: string } } | { ok: false, erro: string }}
 */
export function parseAvisoEvento(body) {
  if (!body || typeof body !== "object") return { ok: false, erro: "corpo inválido" };

  const projeto = texto(body.projeto, 40);
  if (!rotulo(PROJETOS, projeto)) return { ok: false, erro: `projeto desconhecido: ${projeto || "(vazio)"}` };

  const titulo = texto(body.titulo, 120);
  if (!titulo) return { ok: false, erro: "titulo é obrigatório" };

  const corpo = texto(body.texto, Infinity);
  const caminho = texto(body.caminho, 201);
  if (caminho && !/^\/[A-Za-z0-9/_-]{0,199}$/.test(caminho)) return { ok: false, erro: "caminho inválido" };
  ...
  return {
    ...
    aviso: {
      projeto,
      titulo,
      texto: corpo.length > MAX_TEXTO ? `${corpo.slice(0, MAX_TEXTO - 1).trimEnd()}…` : corpo,
      caminho,
      acao: texto(body.acao, 40) || "Abrir no painel",
    },
  };
}

/** Texto do aviso de evento: título com o projeto, o texto da origem e o link, se houver caminho. */
export function avisoDeEvento(aviso) {
  const { nome, base } = PROJETOS[aviso.projeto];
  const titulo = aviso.titulo.includes(nome) ? aviso.titulo : `${aviso.titulo} · ${nome}`;
  return [
    `<b>${escaparHtml(titulo)}</b>`,
    aviso.texto && escaparHtml(aviso.texto),
    aviso.caminho && `<a href="${base}${aviso.caminho}">${escaparHtml(aviso.acao)}</a>`,
  ]
    .filter(Boolean)
    .join("\n");
}
