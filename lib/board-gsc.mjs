...
import { CATALOGO, CLASSES, MEDIDO_POR, RESSALVA_DO_COLETOR, limiarEmTexto, regua } from "./gsc-delta.mjs";
 ...
 */
/**
 * 040 — o nome do grupo do título, UMA vez.
 *
 * Ele aparece em quatro lugares (três caminhos em `GRUPOS`, a nota do grupo) e a tela do mapa
 * precisa dele para achar o nó, porque grupo não tem id literal — `mapaDoBoard()` gera o dele por
 * hash do caminho. Uma quinta cópia escrita à mão em `app/gsc/mapa/[slug]/page.tsx` sobreviveria intacta
 * à correção de uma vírgula aqui, e o nó simplesmente deixaria de ser achado, calado.
 */
export const GRUPO_DO_TITULO = "4. Taxa de Integridade do Título (Sem Truncamento / Sem Reescrita pelo Google)";
...
export const GRUPOS = {
  ...
  larguraTitulo: [GRUPO_DO_TITULO, "Metas recomendadas"],
  reescritaTitulo: [GRUPO_DO_TITULO, "Metas recomendadas"],
  termoNoTitulo: [GRUPO_DO_TITULO, "Metas recomendadas"],
...
export const NOTA_DO_GRUPO = {
  [GRUPO_DO_TITULO]: "O que mede: Snippets que são exibidos exatamente com a copy pensada para conversão.",
};
...
export function mapaDoBoard() {
  ...
  const ramos = RAMOS.map((ramo) => {
    ...
    for (const chave of doRamo) {
      ...
      const s = selo(chave);
      const b = BOARD[chave];
      // 051 — a classe é a SEGUNDA etiqueta, depois da procedência que qualifica todo o resto. A
      // alavanca nomeia a ação semanal ali mesmo: é o que a equipe faz na segunda-feira, e nota
      // exige clique.
      const { classe, acaoSemanal } = CATALOGO[chave];
      const etiquetaDaClasse = classe ? [acaoSemanal ? `${CLASSES[classe].rotulo} · ${acaoSemanal}` : CLASSES[classe].rotulo] : [];
      destino.push({
        id: chave,
        topic: b.titulo,
        tags: [`${s.marca} ${s.rotulo}`, ...etiquetaDaClasse, ...(DIVERGENCIAS[chave] ? [DIVERGENCIAS[chave].tag] : [])],
        ...
        note: [s.detalhe, RESSALVA_DO_COLETOR[chave] && `⚠ o coletor ${RESSALVA_DO_COLETOR[chave]}`, classe && CLASSES[classe].nota]
          .filter(Boolean)
          .join(" · "),
