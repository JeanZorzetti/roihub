...
import { CATALOGO, MEDIDO_POR, regua } from "./gsc-delta.mjs";
...
export function selo(chave) {
  ...
  if (r.tem) {
    const limiar =
      r.meta === null ? "" : Array.isArray(r.meta) ? ` · limiar ${r.meta[0]} a ${r.meta[1]}` : ` · limiar ${r.meta}`;
    return {
      marca: "◆",
 ...
 * @returns {NoDoMapa}
 */
function noDeDetalhe(d, prefixo, i) {
  /** @type {NoDoMapa} */
  const no = { id: id(prefixo, "d", String(i)), topic: d.rotulo };
  if (d.nota) no.note = d.nota;
  if (d.filhos?.length) no.children = d.filhos.map((f, j) => noDeDetalhe(f, `${prefixo}-${i}`, j));
  return no;
}
  ...
  const ramos = RAMOS.map((ramo) => {
    ...
    for (const chave of doRamo) {
      ...
      destino.push({
        ...
        expanded: false,
        children: b.detalhe.map((d, i) => noDeDetalhe(d, chave, i)),
      });
    }
