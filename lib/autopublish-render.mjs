...
// Capa, FAQ e "related" saem do MESMO frontmatter que o layout do projeto já lê. Onde o
// layout os renderiza, emiti-los no corpo publica o bloco duas vezes na mesma página — foi
// o que aconteceu no context. `layoutRenders` declara, por projeto, o que o layout já faz.
const layoutRenders = (project, block) => (project?.layoutRenders ?? []).includes(block);

function markdownBody(draft, project, imageSrc, escapeText = safeMarkdown) {
  const label = labelsFor(project);
  const lines = layoutRenders(project, "hero")
  ...
  if (draft.faqs?.length && !layoutRenders(project, "faq")) {
  ...
  if (draft.relatedSlugs?.length && !layoutRenders(project, "related")) {
