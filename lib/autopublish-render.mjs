...
function markdownBody(draft, imageSrc, escapeText = safeMarkdown) {
  const lines = [
  ...
  if (draft.relatedSlugs?.length) {
    lines.push("## Related guides", ...draft.relatedSlugs.map((slug) => `- [${escapeText(slug)}](/blog/${encodeURIComponent(slug)})`));
  }
...
// tempoLeituraMin, imagem (asset local), imagemAlt. produtosRelacionados é enum fechado
// de slugs reais, então fica vazio — o default do schema aceita.
function renderAstroContentPtBr(draft, project, image, mdxPath) {
  ...
  const frontmatter = [
    ...
    `descricao: ${quoted(String(draft.description ?? "").slice(0, 160))}`,
    `intencao: ${quoted(draft.primaryKeyword)}`,
    `resumo: ${quoted(draft.bluf)}`,
    `publicadoEm: ${draft.publishedAt}`,
    `tempoLeituraMin: ${Math.max(1, Math.round(words / 200))}`,
