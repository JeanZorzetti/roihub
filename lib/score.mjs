...
/**
 * Ordem do ranking: stand-by no fim, depois score, depois curado acima de não-curado.
 *
 * Stand-by ANTES do score de propósito — é decisão do Jean, e decisão vence medição. Sem essa
 * cláusula um projeto parado voltaria a ser o foco do dia sozinho no dia em que o site caísse
 * (`decayEff` vira 10 com health falso), que é exatamente o oposto de estar em stand-by.
 * @param {{standby?:string|null, score:number, curated?:boolean}} a
 * @param {{standby?:string|null, score:number, curated?:boolean}} b
 */
export function ordemDoRanking(a, b) {
  return (
    Number(Boolean(a.standby)) - Number(Boolean(b.standby)) ||
    b.score - a.score ||
