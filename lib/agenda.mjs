 ...
 * Entre ações o sort é estável, então o que sobrevive ao empate é a ordem do ranking.
 */
...
export function filtrosAtivos(f) {
  return ["q", "projeto", "urgencia", "origem", "responsavel"].filter((k) => f[k]);
}
...
/** @param {{titulo:string, projeto:string|null, desc?:string|null, bucket:string, taskId:number|null, task?:{responsavel:string|null}}[]} items */
...
/** Ação do ranking primeiro, depois urgência e data — o desempate de toda ordem. */
export function porUrgencia(a, b) {
  const d = ordemDaLinha(a) - ordemDaLinha(b);
