...
/**
 * O que o robots.txt diz sobre o GPTBot, nos três estados que importam: `ausente`, `barrado` e
 * `permitido`.
 *
 * 🚩 Medido em 01/08: `/GPTBot/i.test(corpo)` aprovava o `orion`, que servia `User-Agent: GPTBot`
 * seguido de `Disallow: /` — o site inteiro fora do ChatGPT, e o check lendo isso como
 * conformidade. **Grep mede a palavra, não a permissão**, a mesma classe de "palavra ≠ URL" do
 * `gateways.mjs`.
 *
 * `barrado` é só o bloqueio TOTAL (`Disallow: /` sem nenhum `Allow:` no grupo). Política parcial
 * — o `reviewshield` libera `/blog` e `/llms.txt` e barra o resto — é decisão do projeto, e um
 * check que a reprovasse estaria opinando sobre escopo, não medindo a norma.
 */
export function julgarGptbot(corpo) {
  // Um grupo = User-agent consecutivos + as regras que os seguem. Comentário e linha que não é
  // nem User-agent nem regra (Sitemap:, Host:) encerram o grupo.
  const grupos = [];
  let atual = null;
  for (const bruta of (corpo ?? "").split(/\r?\n/)) {
    ...
    const regra = l.match(/^(allow|disallow):\s*(.*)$/i);
    if (regra && atual) atual.regras.push({ tipo: regra[1].toLowerCase(), caminho: regra[2].trim() });
    else if (l) atual = null;
  }

  const grupo = grupos.find((g) => g.agentes.includes("gptbot"));
  if (!grupo) return "ausente";
  const barraTudo = grupo.regras.some((r) => r.tipo === "disallow" && r.caminho === "/");
  const liberaAlgo = grupo.regras.some((r) => r.tipo === "allow" && r.caminho);
  return barraTudo && !liberaAlgo ? "barrado" : "permitido";
}
...
/**
 * `url` e `redirecionada` são ADITIVOS (024/D13) e existem porque os dados já estavam no
 * `Response` do fetch (`r.url`, `r.redirected`) e eram descartados. A alternativa era uma segunda
 * requisição com `{ manual: true }` por URL — o dobro de rede para saber o que a primeira resposta
 * já sabia. Os cinco chamadores atuais desestruturam campos nomeados (`corpo`, `status`,
 * `headers`, `erro`), então acrescentar dois não muda nada para eles.
 *
 * No caminho de ERRO os dois vêm preenchidos também: `url` é a URL pedida e `redirecionada` é
 * `false`, nunca `undefined` — quem grava a coluna `redirecionada NOT NULL` não pode receber
 * ausência disfarçada de negativa.
 */
export async function buscar(url, { manual = false } = {}) {
  try {
    ...
    // A URL FINAL, depois dos redirecionamentos: é o destino que conta, e o fato de o sitemap
    // declarar uma URL que redireciona é o achado (FR-013).
    return { status: r.status, headers, corpo: await r.text(), url: r.url || url, redirecionada: !!r.redirected };
  } catch (e) {
    // `error.cause.code` antes de culpar a rede: o undici desiste em 300 s por conta própria.
    return { erro: e.cause?.code || e.code || e.message, url, redirecionada: false };
  }
}
...
export const CHECKS = [
  ...
  },
  {
    ...
    async rodar(ctx) {
      ...
      const faltas = [];
      if (!ctx.robots.erro) {
        const gptbot = julgarGptbot(ctx.robots.corpo);
        if (gptbot === "ausente") faltas.push("robots.txt sem GPTBot");
        // Barrar é PIOR que não declarar, e o grep antigo lia os dois como conformidade.
        if (gptbot === "barrado") faltas.push("robots.txt BARRA o GPTBot (Disallow: /)");
      }
