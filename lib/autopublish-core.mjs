...
export function validateDraft(draft, project) {
  ...
  const intent = [draft?.title, draft?.description, draft?.primaryKeyword].join(" ");
  ...
  if (project?.risk === "ymyl-restricted" && /\b(?:botox|dosage|dose|diagnos(?:is|e)|prescri(?:be|ption)|treat(?:ment)?|cure)\b/i.test(intent)) invalid.add("ymyl");
