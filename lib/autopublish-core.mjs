...
const operationalIntent = /\b(?:operations?|operational|software|platform|workflow|automation|scheduling|scheduler|appointments?|booking|intake|forms?|crm|practice management|clinic management|administrative|administration|front desk|patient communication|reminders?|follow up|billing|payments?|invoicing|inventory|staff|team|reviews?|reputation|marketing|lead management|analytics|reporting|retention|no shows?|customer service|client management)\b/;
const clinicalGuidance = /\b(?:botox|fillers?|inject(?:ion|able|ed|ing)?|dosage|dose|diagnos(?:is|e)|prescri(?:be|ption)|medications?|medicine|treat(?:ment|ed|ing)?|cure|symptoms?|contraindications?|side effects?|swelling|swollen|bruis(?:e|ing)|cold compress(?:es)?|ice packs?|infection|allergic|anaphylaxis|bleeding|wounds?|fever|pain|antibiotics?|ibuprofen|paracetamol|acetaminophen|aspirin|pregnan(?:t|cy)|emergency)\b/;

function editorialContent(draft) {
  return [
    draft?.title,
    draft?.description,
    ...
    ...(Array.isArray(draft?.faqs)
      ? draft.faqs.flatMap((faq) => [faq?.q, faq?.a, faq?.question, faq?.answer])
      : []),
  ].map(normalizeIntent).filter(Boolean).join(" ");
}
...
export function validateDraft(draft, project) {
  ...
  if (project?.risk === "ymyl-restricted") {
    const intent = [draft?.title, draft?.description, draft?.primaryKeyword]
      ...
      .filter(Boolean)
      .join(" ");
    if (!operationalIntent.test(intent) || clinicalGuidance.test(editorialContent(draft))) invalid.add("ymyl");
  }
