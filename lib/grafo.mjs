...
/**
 * Aponta cada aresta para onde o href CAIU. A página é guardada pelo destino (D13) e a aresta pelo
 * href: sem esta troca, todo link que passa por redirect termina numa URL sem página, e a travessia
 * quebra ali.
 *
 * MEDIDO no Sirius em 23/09/2026: `/blog` linka `/pt-BR/blog/x`, que responde 307 para `/blog/x`.
 * 26 posts linkados saíam órfãos, e o voto do link ia para a URL que redireciona. Só roda depois da
 * travessia: a fronteira precisa do href cru para fechar (ver `fronteira`).
 *
 * @param {{de:string, para:string, ancora:string}[]} arestas @param {Map<string,string>} destinos pedida → destino
 */
export function resolverArestas(arestas, destinos) {
  return arestas.map((a) => ({ ...a, para: destinos.get(a.para) ?? a.para }));
}
