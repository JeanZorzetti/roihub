...
export function carregarCorpus({ memoriaDir = MEMORIA_PADRAO } = {}) {
  ...
  // Handoff em SUBPASTA entra, e o id carrega a pasta. `handoff/` ganhou subprojeto em 01/09
  // (`funil-seo/`) e o `readdirSync` sem recursão o ignorava em SILÊNCIO: nome de pasta não
  // termina em `.md`, então não havia erro nenhum — só documentos faltando na busca, que é o
  // modo de falha mais caro desta base.
  //
  // ⚠️ Arquivo na RAIZ mantém o id NU (`handoff-atma-reindexado.md`). É o vocabulário das
  // `fontes` do dourado e do `existsSync` de `test/dourado.test.mjs`; prefixar tudo zeraria o
  // recall de toda pergunta que cita handoff, sem erro nenhum aparecer.
  //
  // ⚠️ Separador normalizado para `/`: o índice é gerado no Windows (`indexar.mjs` só roda nesta
  // máquina) e lido pelo container Linux. `funil-seo\x.md` e `funil-seo/x.md` são DUAS chaves na
  // PK de `hub_corpus`, e a divergência só apareceria como documento sumido.
  for (const f of readdirSync(HANDOFF, { recursive: true })) {
    if (!f.endsWith(".md")) continue;
    const id = f.split(sep).join("/");
    docs.push({ id, tipo: "handoff", titulo: id, texto: readFileSync(join(HANDOFF, f), "utf8") });
  }
