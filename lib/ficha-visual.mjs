 ...
 *
 * `fracao` é do TOTAL apurado (a mesma soma que `totalComposto()` em lib/ficha.mjs calcula pro
 * "total composto" logo abaixo), não do maior canal (achado 5 do design-review de 03/09): contra
 * o maior, orgânico enchia 100% do trilho enquanto o texto ao lado dizia "total composto 657" —
 * lido como composição, mentia por até dezenas de pontos percentuais.
 *
...
export function canaisDoN4(celulas) {
  ...
  const total = doCanal.filter(ehBarra).reduce((soma, c) => soma + c.valor, 0);
  const canais = [
    ...doCanal.filter(ehBarra).sort((a, b) => b.valor - a.valor).map((c) => ({ celula: c, fracao: total > 0 ? c.valor / total : 0 })),
