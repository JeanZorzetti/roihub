...
/**
 * The one line of a step with no entry (055). In the glossary's words: "sem ação", never "dentro",
 * "ok" or "✓", and absence as "∅ sem ação · <motivo>" — Sirius's step 2 was empty because its five
 * leaves had no reading, and the old footer "Sem ação: 2 · Desempenho" read as a pass.
 *
 * @param {{contagem: {semDisparo: number, naoDecide: number, semLeitura: number},
 *          semLeituraPorMotivo: {motivo: string, chaves: string[]}[]}} g @param {(chave: string) => string} nomeDe
 */
export function textoDoDegrauVazio(g, nomeDe) {
  const { semDisparo, naoDecide, semLeitura } = g.contagem;
  const total = semDisparo + naoDecide + semLeitura;
  if (!total) return "sem ação · nenhuma folha com regra neste degrau";
  const folhas = `${total} ${total === 1 ? "folha" : "folhas"}`;
  const leitura = g.semLeituraPorMotivo.map((x) => `${eLista(x.chaves.map(nomeDe))} (${x.motivo})`).join("; ");
  if (semLeitura === total) return `∅ sem ação · ${folhas} sem leitura: ${leitura}`;
  if (semDisparo === total) return total === 1 ? "sem ação · a folha do degrau não disparou" : `sem ação · nenhuma das ${folhas} disparou`;
  const partes = [
    semDisparo ? `${semDisparo} sem disparo` : "",
    naoDecide ? `${naoDecide} em que a amostra não decide` : "",
    semLeitura ? `${semLeitura} sem leitura: ${leitura}` : "",
  ].filter(Boolean);
  return `sem ação · ${partes.join(", ")}`;
}

/**
 * @typedef {{projeto: string, alavanca: string, responsavel: string, marcado: string, reler: string,
 *            leituras: Record<string, string>}} Marca
 */

const DIAS_DE_RELEITURA = [7, 14, 28];
const tem = (obj, k) => Object.hasOwn(obj, k);

/**
 * The whole validation of the "done" form (055). The reading "at the mark" comes from the form, not
 * recomputed by the action: recomputing would redo the map's opening (GSC, crawl, CrUX) on every
 * click (research D1). `Object.hasOwn`, not `in`: "toString" is `in` every object.
 *
 * @param {Record<string, unknown>} c @param {{slugs: string[], hoje: string}} ctx @returns {Marca|null}
 */
export function lerMarca(c, { slugs, hoje }) {
  const dias = Number(c.dias);
  if (!slugs.includes(String(c.projeto)) || !tem(ALAVANCAS, String(c.alavanca))) return null;
  if (!RESPONSAVEL_IDS.includes(String(c.responsavel)) || !DIAS_DE_RELEITURA.includes(dias)) return null;
  let leituras;
  try {
    leituras = JSON.parse(String(c.leituras));
  } catch {
    return null;
  }
  if (!leituras || typeof leituras !== "object" || Array.isArray(leituras)) return null;
  const pares = Object.entries(leituras);
  if (pares.length > 32 || pares.some(([k, v]) => !tem(REGRAS, k) || typeof v !== "string" || v.length > 300)) return null;
  return { projeto: String(c.projeto), alavanca: String(c.alavanca), responsavel: String(c.responsavel), marcado: hoje, reler: addDaysISO(hoje, dias), leituras };
}
 ...
 *
 * 055: all five steps, always. A hidden step read as a lost task on Sirius (23/09/2026: "1" then
 * "3"), and it was hidden for lack of READINGS, not because it passed. A mark only reorders and dims;
 * it never touches a dispatch, so the map nodes keep saying what the rule says (FR-010).
 *
 * @param {Record<string, Disparo>} disparos @param {{marcas?: Marca[], hoje?: string|null}} [opts]
 */
export function plano(disparos, { marcas = [], hoje = null } = {}) {
  /** @type {Disparo[]} */
  ...
  const marcaDe = new Map(marcas.filter((m) => tem(ALAVANCAS, m.alavanca)).map((m) => [m.alavanca, m]));
  // A marked lever with no dispatch today stays when a reading it was marked on failed: otherwise the
  // GSC being down on the day passes for solved (research D5).
  /** @type {Map<string, string>} */
  const falhou = new Map();
  for (const [id, m] of marcaDe) {
    if (porAlavanca.has(id)) continue;
    const semLeitura = Object.keys(m.leituras).map((k) => disparos[k]).filter((d) => d?.estado === "sem-leitura");
    if (!semLeitura.length) continue;
    porAlavanca.set(id, semLeitura);
    falhou.set(id, [...new Set(semLeitura.map((d) => d.motivo))].join("; "));
  }

  const entradas = [...porAlavanca].map(([id, crus]) => {
    const m = marcaDe.get(id) ?? null;
    const motivos = crus.map((d) => ({ ...d, naMarca: m ? (m.leituras[d.chave] ?? null) : null, novo: m ? !tem(m.leituras, d.chave) : false }));
    const leituraFalhou = falhou.get(id) ?? null;
    // Past the deadline, only a rule that still FIRES comes back; an unread one cannot say so.
    const apresentacao = !m ? "ativa" : !leituraFalhou && hoje !== null && hoje >= m.reler ? "voltou" : "aguardando";
    ...
    return {
      ...
      apresentacao,
      marca: m ? { responsavel: m.responsavel, marcado: m.marcado, reler: m.reler } : null,
      leituraFalhou,
    };
  });

  const aguarda = (e) => e.apresentacao === "aguardando";
  const degraus = DEGRAUS.map((g) => {
    const doDegrau = entradas
      .filter((e) => ALAVANCAS[e.alavanca].degrau === g.id)
      .sort(
        (a, b) =>
          Number(aguarda(a)) - Number(aguarda(b)) ||
    ...
    // A leaf belongs to the step of its rule's lever. The checklist (rule `null`) belongs to none: it
    // is also "sem-acao", and counting a procedure as "inside" repeats the defect this spec opened on.
    const folhas = ds.filter((d) => d.alavanca && ALAVANCAS[d.alavanca].degrau === g.id);
    /** @type {Map<string, string[]>} */
    const porMotivo = new Map();
    for (const d of folhas.filter((d) => d.estado === "sem-leitura")) porMotivo.set(d.motivo, [...(porMotivo.get(d.motivo) ?? []), d.chave]);
    return {
      ...g,
      estado: !doDegrau.length ? "vazio" : doDegrau.every(aguarda) ? "aguardando" : "com-acao",
      entradas: doDegrau,
      contagem: {
        semDisparo: folhas.filter((d) => d.estado === "sem-acao").length,
        naoDecide: folhas.filter((d) => d.estado === "nao-decide").length,
        semLeitura: folhas.filter((d) => d.estado === "sem-leitura").length,
      },
      semLeituraPorMotivo: [...porMotivo].map(([motivo, chaves]) => ({ motivo, chaves })),
    };
  });
  const primeiro = degraus.flatMap((g) => g.entradas.map((e) => ({ degrau: g.id, e }))).find(({ e }) => !aguarda(e));

  return {
    degraus,
    primeira: primeiro ? { degrau: primeiro.degrau, alavanca: primeiro.e.alavanca } : null,
